...
const createTable = (data, workoutTitle, workoutDescription, containerId) => {
  ...
  data.forEach(entry => {
    ...
    plusRepButton.addEventListener('click', () => {
      ...
      reps += 2;
      repValue.textContent = reps;
      saveInputsToLocalStorage(containerId, entry.exercise, setsCell.textContent, reps, weight);
    });
...
const saveInputsToLocalStorage = (containerId, exercise, sets, reps, weight) => {
